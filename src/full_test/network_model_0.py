...
def generate_Conv_Network(images, batch_size, n_classes, batch_norm=True, is_training=True):
    """
    DESCRIPTION
    ...
    with tf.variable_scope('layer_3_hidden'):
        ...
        hid_3 = nl.gen_hidden_layer(rehaped_conv_output, [flattened_dim, 4096], [4096], batch_norm=batch_norm, is_training=is_training)

    # Now add the output layer
    #with tf.name_scope('layer_4_output'):
    with tf.variable_scope('layer_4_output'):
        net_output = nl.gen_output_layer(hid_3, [4096, n_classes], [n_classes])
